...
PT_FILES_DIR = "matrix"
OUTPUT_MODEL_DIR = "cvae_pytorch_models_AD_CN_classifier_phd_v3" # Updated output dir
...
C_MAX = 25.0 # Max capacity for KL divergence
C_ANNEAL_EPOCHS = int(BETA_ANNEAL_EPOCHS * 0.8) # Epochs to reach C_MAX
...
EARLY_STOPPING_PATIENCE_CLF = 20 
...
class EarlyStopping:
    ...
    def __call__(self, current_val_metric, model):
        ...
        elif score < self.best_score + self.delta: # Check if performance has not improved
            self.counter += 1
...
class ConnectomeDataset(Dataset):
    def __init__(self, pt_file_paths_override, selected_metrics, pathology_mapping, 
        ...
        pathologies_ohe_np = np.array(self.pathologies_mapped_raw).reshape(-1,1) # For CVAE condition
        ...
        if fit_scalers:
            ...
            self.condition_preprocessors = {
                ...
                'pathology_ohe_encoder_for_cvae': self.pathology_encoder_ohe_for_cvae, # Store this specific encoder
        ...
        # Conditions for CVAE (age, sex, and 3-class pathology OHE)
    ...
    def __getitem__(self, idx):
        ...
        # Return items needed by the CVAE training loop and the classifier data extraction
...
class CVAE_ResNetEncoder(nn.Module):
    def __init__(self, img_channels, img_res, condition_dim, latent_dim,
        ...
        encoder_modules.extend([
            ...
            nn.BatchNorm2d(initial_conv_filters), nn.ReLU(inplace=True)])
...
def train_cvae_epoch(model, dataloader, optimizer, scaler, current_beta_kl, current_capacity_c, device):
    ...
    for matrices, conditions, _, _ in dataloader: # Adjusted unpacking
...
if __name__ == "__main__":
    ...
    for epoch in range(1, CVAE_EPOCHS + 1):
        ...
        current_capacity_c = min(C_MAX, C_MAX * (epoch / C_ANNEAL_EPOCHS) ) if C_ANNEAL_EPOCHS > 0 else C_MAX
        
        train_loss, train_recon, train_kl = train_cvae_epoch(cvae_model, train_loader_cvae, optimizer_cvae, cvae_grad_scaler, current_beta_kl, current_capacity_c, DEVICE) # Removed C_MAX
        val_loss, val_recon, val_kl = validate_cvae_epoch(cvae_model, val_loader_cvae, current_beta_kl, current_capacity_c, DEVICE) # Removed C_MAX
        scheduler_cvae.step() 
    ...
    def extract_latent_and_filter(dataset_subset):
        ...
        for i in range(len(dataset_subset)): 
            matrix, condition, binary_label, pathology_str = dataset_subset[i] # Adjusted unpacking
    ...
    # Tune decision threshold based on validation set (Point 3 of review)
    ...
    _, _, val_true_labels_for_thresh, val_probs_ad_for_thresh, _ = evaluate_binary_classifier(
        binary_classifier, val_loader_clf, criterion_binary_clf, DEVICE) # Use val_loader_clf
    ...
    if len(np.unique(val_true_labels_for_thresh)) > 1 and len(val_probs_ad_for_thresh) > 0 : 
        ...
        if len(youden_j) > 0 : 
            ...
            logger.warning("Could not determine Youden's J (thresholds array might be degenerate). Using 0.5.")
    else:
        logger.warning("Not enough data or only one class present in validation set for threshold tuning. Using 0.5.")
    ...
    # --- Sanity Check: Logistic Regression on Latent Features (Point 4 of review) ---
    ...
    try:
        if len(train_latent_clf) > 0 and len(np.unique(train_labels_clf.cpu().numpy())) > 1:
            ...
            if len(val_latent_clf) > 0 and len(np.unique(val_labels_clf.cpu().numpy())) > 1:
                ...
                logger.warning("Skipping Logistic Regression validation AUC: Insufficient samples or only one class in validation set.")
            ...
            if len(test_latent_clf) > 0 and len(np.unique(test_labels_clf.cpu().numpy())) > 1:
                ...
                logger.warning("Skipping Logistic Regression test evaluation: Insufficient samples or only one class in test set.")
        else:
            logger.warning("Skipping Logistic Regression sanity check: Insufficient samples or only one class in training set.")
    except Exception as e:
        logger.error(f"Error during Logistic Regression sanity check: {e}")
